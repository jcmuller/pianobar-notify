...
I have a call to C<_show_in_awesome>. If you are not using L<Awesome|http://awesome.naquadah.org/>, or don't have `musiccover` defined in your F<rc.lua> (F<~/.config/awesome/rc.lua>), comment out the line in C<_display> that calls it (currently 144, but it will change). 