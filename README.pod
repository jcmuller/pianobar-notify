...
I have a call to `_show_in_awesome`. If you are not using L<Awesome|http://awesome.naquadah.org/>, or don't have `musiccover` defined in your `rc.lua`, comment out the line in `_display` that calls it (currently 144, but it will change). 